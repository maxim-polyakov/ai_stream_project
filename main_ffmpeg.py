...
class FFmpegStreamManager:
    ...
    def _cleanup_sent_files(self):
        """Очистка файлов, которые были отправлены"""
        try:
            if not self.use_mpegts_cache or not self.mpegts_cache:
                return

            current_time = time.time()
            max_age_hours = 24  # Удаляем файлы старше 24 часов
            ...
            for cache_key, cache_info in self.mpegts_cache.items():
                ...
                # Если файл был отправлен более X часов назад
                if last_accessed > 0 and (current_time - last_accessed) > (max_age_hours * 3600):
                    mpegts_path = os.path.join(self.mpegts_cache_dir, cache_info['filename'])
                    if os.path.exists(mpegts_path):
            ...
            for cache_key, mpegts_path, cache_info in files_to_remove:
                try:
                    ...
                    logger.error(f"❌ Ошибка удаления файла {mpegts_path}: {e}")

            if files_to_remove:
                self._save_mpegts_cache_index()
                logger.info(f"🧹 Очищено {len(files_to_remove)} старых файлов из кэша")
    ...
    def _stream_controller(self):
        ...
        try:
            ...
            while self.is_streaming and not self._controller_stop_event.is_set():
                ...
                # Сортируем файлы по времени создания (старые первыми)
                cache_items = list(self.mpegts_cache.items())
    ...
    def _delete_cache_file(self, file_info):
        ...
        try:
            ...
            logger.info(f"🗑️ Удален файл из кэша: {file_info['filename']}")
            return 1
        except Exception as e:
    ...
    def _send_mpegts_data(self, mpegts_path: str, duration: float) -> bool:
        """
        Отправка MPEG-TS файла через pipe в FFmpeg с защитой от падений

        Args:
            mpegts_path: Путь к MPEG-TS файлу
            duration: Ожидаемая длительность файла в секундах

        Returns:
            True если файл успешно отправлен, False в случае ошибки
        """
        if not self.is_streaming:
            logger.warning("⚠️ Пытаемся отправить данные, но стрим не активен")
            return False

        if not self.ffmpeg_stdin:
            logger.error("❌ FFmpeg stdin недоступен")
            return False
        ...
        try:
            ...
            logger.info(f"📤 Начинаю отправку: {filename} ({file_size / 1024:.1f} KB, {duration:.1f} сек)")

            # Рассчитываем целевую скорость отправки (байт/сек)
            # Используем реальную длительность, если она известна, иначе используем переданную
            try:
                # Пытаемся получить реальную длительность файла
                cmd = ['ffprobe', '-v', 'error', '-show_entries', 'format=duration',
                       '-of', 'default=noprint_wrappers=1:nokey=1', mpegts_path]
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=5)
                if result.returncode == 0 and result.stdout.strip():
                    actual_duration = float(result.stdout.strip())
                    if 0.1 < actual_duration < 3600:  # Реалистичные границы
                        duration = actual_duration
            except:
                pass  # Используем переданную длительность

            target_bytes_per_second = file_size / duration if duration > 0 else file_size / 10.0
            ...
            with open(mpegts_path, 'rb') as f:
                ...
                # Оптимальный размер чанка для MPEG-TS
                # MPEG-TS пакеты по 188 байт, берем кратно
                chunk_size = 188 * 350  # ~65.8KB - оптимально для скорости и управления

                logger.debug(f"📊 Параметры отправки: скорость={target_bytes_per_second / 1024:.1f} KB/сек, "
                             f"чанк={chunk_size / 1024:.1f} KB")

                # Используем блокировку для безопасного доступа к stdin
                with self.stdin_lock:
                    while bytes_sent < file_size and self.is_streaming:
                        # Периодически проверяем живой ли FFmpeg
                        if bytes_sent > 0 and bytes_sent % (188 * 10000) == 0:  # Каждые ~10k пакетов
                            if not self._check_ffmpeg_alive():
                                logger.error("❌ FFmpeg завершился во время отправки")
                                self.is_streaming = False
                                return False
                        ...
                        chunk = f.read(chunk_size)
                        if not chunk:
                            # Если не смогли прочитать, но еще не дошли до конца файла
                            if bytes_sent < file_size:
                                logger.warning(f"⚠️ Неожиданный конец файла: {bytes_sent}/{file_size} байт")
                                # Пробуем прочитать остаток другим способом
                        ...
                        try:
                            # Отправляем чанк в FFmpeg
                            self.ffmpeg_stdin.write(chunk)
                            bytes_sent += len(chunk)

                            # Периодически сбрасываем буфер (но не слишком часто)
                            current_time = time.time()
                            if current_time - last_flush_time > 0.1:  # Каждые 100мс
                                try:
                                    self.ffmpeg_stdin.flush()
                                except:
                                    pass  # Игнорируем ошибки flush
                                last_flush_time = current_time

                            # Синхронизируем скорость отправки с реальным временем
                            ...
                            if target_bytes_per_second > 0:
                                ...
                                if elapsed < expected_time:
                                    ...
                                    # Не спим слишком долго за один раз, чтобы не блокировать
                                    if sleep_time > 0.001:
                                        time.sleep(min(sleep_time, 0.05))

                            # Логируем прогресс каждые 2 секунды или 10%
                            if current_time - last_log_time > 2.0:
                                progress = (bytes_sent / file_size) * 100
                                elapsed_str = f"{elapsed:.1f}/{duration:.1f} сек"
                                actual_speed = bytes_sent / elapsed / 1024 if elapsed > 0 else 0

                                logger.info(f"📊 Прогресс: {progress:.1f}% ({bytes_sent}/{file_size} байт) - "
                                            f"{elapsed_str}, скорость: {actual_speed:.1f} KB/сек")
                                ...
                                try:
                                    socketio.emit('stream_progress', {
                                        ...
                                        'elapsed': elapsed,
                                        'duration': duration,
                                        'speed_kbps': actual_speed * 8,  # В kbps
                                        'timestamp': datetime.now().isoformat()
                                    })
                                except:
                                    pass
                            ...
                            # Пробуем получить причину ошибки из stderr
                            try:
                                if self.stream_process:
                                    stderr_data = self.stream_process.stderr.read()
                                    if stderr_data:
                                        logger.error(
                                            f"FFmpeg stderr: {stderr_data.decode('utf-8', errors='ignore')[:500]}")
                            except:
                                pass
                            ...
                            if e.errno == 32:  # Broken pipe на уровне ОС
                                logger.error("❌ OSError: Broken pipe (errno 32)")
                                self.is_streaming = False
                                return False
                            else:
                                logger.error(f"❌ OSError при отправке: {e}")
                                return False
                    ...
                    # Финальный flush после отправки всего файла
                    try:
                        if self.ffmpeg_stdin:
                            self.ffmpeg_stdin.flush()
                    except:
            ...
            if success_ratio >= 0.99:  # 99% успешно
                logger.info(f"✅ Файл успешно отправлен за {total_elapsed:.1f} сек "
                            f"(ожидалось {duration:.1f} сек)")
                logger.info(f"📊 Итог: {bytes_sent}/{file_size} байт ({success_ratio * 100:.1f}%), "
                            f"скорость: {file_size / total_elapsed / 1024:.1f} KB/сек")
                return True

            elif success_ratio >= 0.90:  # 90% успешно
                logger.warning(f"⚠️ Файл частично отправлен: {bytes_sent}/{file_size} байт "
                               f"({success_ratio * 100:.1f}%)")
                logger.warning(f"📊 Время: {total_elapsed:.1f} сек (ожидалось {duration:.1f} сек)")
                # Для непрерывности стрима считаем успехом
                return True

            elif success_ratio >= 0.80:  # 80% успешно
                logger.warning(f"⚠️ Файл отправлен с ошибками: {bytes_sent}/{file_size} байт "
                               f"({success_ratio * 100:.1f}%)")
                logger.warning(f"📊 Время: {total_elapsed:.1f} сек (ожидалось {duration:.1f} сек)")
                # Пробуем продолжить стрим
                return True

            else:
                logger.error(f"❌ Отправка провалилась: {bytes_sent}/{file_size} байт "
                             f"({success_ratio * 100:.1f}%)")
    ...
    def _create_test_stream(self) -> Optional[str]:
        """Создание тестового MPEG-TS файла"""
        try:
            test_mpegts = tempfile.NamedTemporaryFile(suffix='.ts', delete=False)
            test_mpegts.close()
            ...
            if result.returncode == 0 and os.path.exists(test_mpegts.name):
                ...
                return test_mpegts.name

        except Exception as e:
            logger.error(f"❌ Ошибка создания тестового потока: {e}")
    ...
    def _monitor_ffmpeg_with_restart(self):
        """Мониторинг FFmpeg с автовосстановлением и корректной остановкой"""
        try:
            ...
            while self.is_streaming and not self._monitor_stop_event.is_set():
                ...
                if not self.stream_process:
                    logger.warning("⚠️ FFmpeg процесс не существует")
                    time.sleep(1)
                    continue
                ...
                try:
                    ...
                    while True:
                        ...
                        if self._monitor_stop_event.is_set():
                            logger.info("🛑 Мониторинг получил сигнал остановки")
                            return
                        ...
                        if self.stream_process.poll() is not None:
                            ...
                            if return_code != 0 and not self._monitor_stop_event.is_set():
                        ...
                        rlist, _, _ = select.select([self.stream_process.stderr], [], [], 0.5)

                        if not rlist:
                            # Нет данных, проверяем остановку
                            if self._monitor_stop_event.is_set():
                        ...
                        # Проверяем остановку после чтения
                        if self._monitor_stop_event.is_set():
                            return

                        # Обработка строки
                        self._process_ffmpeg_line(line, stream_connected)

                        # Обновляем флаг подключения
                        if 'rtmp://' in line and any(x in line.lower() for x in ['connected', 'publish', 'live']):
                        ...
                        # Обработка критических ошибок
                            ...
                            if not self._monitor_stop_event.is_set():
                                self._handle_critical_error(line, restart_count, max_restarts, last_restart_time)
                                return  # Выходим, чтобы новый монитор мог запуститься
                    ...
                    if self._monitor_stop_event.is_set():
                        return
                    logger.debug(f"Ошибка select/OS: {e}")
                    time.sleep(1)
                except Exception as e:
            ...
            logger.info("🛑 Мониторинг FFmpeg остановлен")

        except Exception as e:
            logger.error(f"❌ Критическая ошибка мониторинга FFmpeg: {e}", exc_info=True)

    def _process_ffmpeg_line(self, line: str, stream_connected: bool):
        """Обработка строки из stderr FFmpeg"""
        try:
            if 'frame=' in line and 'fps=' in line:
                ...
                if 'bitrate=' in line:
                    try:
                        ...
                        if bitrate_match:
                            ...
                            if hasattr(self, '_last_bitrate_log'):
                                if current_time - self._last_bitrate_log > 10:
                                    logger.info(f"📊 Текущий битрейт: {current_bitrate:.1f} kbps")
                                    self._last_bitrate_log = current_time

                                    # Предупреждение о низком битрейте
                ...
                if hasattr(self, '_last_stats_log'):
                    if current_time - self._last_stats_log > 5:
            ...
            logger.debug(f"Ошибка обработки строки FFmpeg: {e}")

    def _handle_critical_error(self, line: str, restart_count: int, max_restarts: int, last_restart_time: float):
    ...
    def stop_stream(self):
        ...
        # 1. Устанавливаем флаги остановки
        self.is_streaming = False

        # 2. Устанавливаем stop events для всех потоков
        if hasattr(self, '_monitor_stop_event'):
            self._monitor_stop_event.set()
            logger.info("✅ Сигнал остановки отправлен монитору FFmpeg")

        if hasattr(self, '_controller_stop_event'):
            self._controller_stop_event.set()
            logger.info("✅ Сигнал остановки отправлен контроллеру")

        if hasattr(self, '_cleanup_stop_event'):
            self._cleanup_stop_event.set()
            logger.info("✅ Сигнал остановки отправлен очистке кэша")
        ...
        try:
            ...
            if hasattr(self, 'stream_process') and self.stream_process:
                try:
                    ...
                    # Ждем до 3 секунд для корректного завершения
                    for _ in range(30):
                        if self.stream_process.poll() is not None:
                            break
                        time.sleep(0.1)
...
class VideoGenerator:
    ...
    def __init__(self, ffmpeg_manager: FFmpegStreamManager = None):
        ...
        self.video_cache_dir = 'video_cache'
        self.avatars_dir = "avatars"  # Добавьте эту строку
        os.makedirs(self.video_cache_dir, exist_ok=True)
    ...
    def create_message_video(self, agent_name: str, message: str,
        ...
        """Создание видео с текстом сообщения, аватаром и сохранение в кэш"""
        try:
            timestamp = int(time.time())
            ...
            # Загружаем аватар агента
            avatar_path = os.path.join(self.avatars_dir, f"{agent_name}.png")
            avatar_img = None
    ...
    async def run_discussion_round(self):
        ...
        try:
            ...
            for agent_idx, agent in enumerate(speaking_order):
                ...
                try:
                    # 1. Генерируем аудио (остается в основном потоке)
                    ...
                    # 2. Создаем видео с сообщением (остается в основном потоке)
                    ...
                    # 3. Генерация MPEG-TS в ОТДЕЛЬНОМ ПОТОКЕ
                    if audio_file and video_message and self.ffmpeg_manager:
                        # Создаем копии переменных для передачи в поток
                        audio_file_path = audio_file
                        video_message_path = video_message
                        duration = message_video_duration

                        # Запускаем генерацию MPEG-TS в отдельном потоке
                        def generate_mpegts_in_thread():
                            try:
                                timestamp = int(time.time())
                                ...
                                if success:
                                    # Добавляем в кэш
                                    cache_key = self.ffmpeg_manager._get_mpegts_cache_key(video_message_path,
                                                                                          audio_file_path)
                                ...
                                logger.error(f"❌ Ошибка генерации MPEG-TS в потоке для {agent.name}: {e}")

                        # Запускаем поток
                        mpegts_thread = threading.Thread(target=generate_mpegts_in_thread, daemon=True)
                        mpegts_thread.start()
