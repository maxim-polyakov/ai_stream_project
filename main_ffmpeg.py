    ...
    def start_stream(self, use_audio: bool = True):
        ...
        try:
            ...
            ffmpeg_cmd = [
                ...
                # Формат вывода с улучшениями для стабильности
                '-f', 'flv',
                ...
                # 🔥 КРИТИЧЕСКИ ВАЖНО: Добавляем параметры для борьбы с "Connection reset by peer"
                '-timeout', '5000000',  # 5 секунд таймаут (в микросекундах)
                '-rw_timeout', '5000000',  # Таймаут чтения/записи
                '-stimeout', '5000000',  # Таймаут сокета
                '-reconnect', '1',  # Включить переподключение
                '-reconnect_streamed', '1',  # Переподключение для потоков
                '-reconnect_on_network_error', '1',  # Переподключение при сетевых ошибках
                '-reconnect_on_http_error', '4xx,5xx',  # Переподключение при HTTP ошибках
                '-reconnect_delay_max', '5',  # Максимальная задержка переподключения
                '-multiple_requests', '1',  # Разрешить множественные запросы

                # Параметры сетевого буфера
                '-buffer_size', '1024k',  # Размер буфера для сетевых операций
